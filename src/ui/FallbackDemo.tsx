  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "auto", background: "var(--bg-0)", padding: "var(--space-3)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ color: "var(--text-0)", margin: 0 }}>{t.fallback.title}</h2>
        <button type="button" onClick={onBack} style={{ padding: "8px 16px", borderRadius: "var(--radius-1)", border: "1px solid var(--stroke)", background: "var(--bg-2)", color: "var(--text-1)", cursor: "pointer" }}>{t.fallback.back}</button>
      </div>
      <p style={{ color: "var(--text-1)" }}>{t.fallback.intro}</p>
      <ol data-testid="fallback-script" style={{ color: "var(--text-1)", lineHeight: 1.7 }}>
        {t.fallback.script.map((s, i) => <li key={i}>{s}</li>)}
      </ol>
